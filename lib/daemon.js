...
class DeviceStream{
  ...
  write(chunk, callback) {
	  if(this.simulator) {
		  this.client.write(chunk,'utf-8',(error)=>{
  ...
  tryRead(size, callback) {
	...
	if(this.buffer.length >= size) {
		...
		//console.log('after tryRead {'+ this.buffer.toString('hex') +'}');
	}
	else {
		this.waitTimeout --;
...
class USBDataDaemon extends Emitter {
  ...
  constructor(device, deviceId, simulator) {
	...
	this.on('sendHeader', () => {
		...
		if(this.porta_out_length || this.portb_out_length) {
		...
		if(totalLength <= 0){
			...
			if(totalLength > 0){
				...
				this.deviceStream.read(totalLength,(error,data)=>{
					...
					if(error) {
						console.log('deviceStream.read error {'+error+'}');
					}
					else {
