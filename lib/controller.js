...
controller.register = function(options) {
  ...
  return new Promise( (resolve) => {
	...
	request.post('http://192.168.56.101:4567/login', options, function(err,httpResponse,body) {
		  
		if (!error && response.statusCode === 200) {
			//callback({success: true, msg: body});
			console.log('request is success ');
			
			let param = {'sn': 'qwertyui','password':'qwertyui','type': 'vessel-usb'};
			let options = {
				json: true,
				header : header,
				jar: true,
			...
			request.post('http://192.168.56.101:4567/registerGadget', options, function(err,httpResponse,body) {
