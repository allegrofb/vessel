...
var Tessel = require('./vessel/vessel');
...
controller.register = function(options) {
  ...
  return new Promise( (resolve) => {
		...
		request.post('http://192.168.56.101:4567/login', options, function(error,httpResponse,body) {
			...
			if (!error && httpResponse.statusCode === 200) {
				...
				let param = {'username': 'qwertyui','password':'qwertyui','type': 'vessel-usb'};
				let options = {
					json: true,
					header : header,
					jar: true,
				...
				request.post('http://192.168.56.101:4567/registerGadget', options, function(error,httpResponse,body) {
...
controller.startDaemon = function(options) {
...
controller.stopDaemon = function(options) {

  return new Promise( (resolve) => {
	getRunningPid((err, pid) => {
		if (!err) {
			process.kill(pid, 'SIGTERM');
			process.stdout.write('Stopping vessel daemon. Goodbye!\n');
		} else {
			process.stdout.write('Vessel daemon is already stopped.\n');
		}
		resolve();
	});
...
controller.exec = function(options) {

  return new Promise( (resolve) => {
	getRunningPid(function (err, pid) {
		if (!err) {
			require('../analog_write')();
		} else {
			process.stdout.write('\n Please start Vessel Daemon\n'.bold);
		}
		resolve();
	});
  });
